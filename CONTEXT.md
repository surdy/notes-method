...
- **Embedded agent chat** *(implemented — ADR 0011)* — A desktop-only feature: the Tauri shell spawns an agent CLI (Claude Code, Codex, or Copilot CLI) locally and streams it to a chat panel over Tauri IPC. The `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream and auto-wires the agent to the active vault's `/mcp/<vault>` (or read-only `/mcp-ro/<vault>`) endpoint, with a read-only default and a read-only/read-write toggle. Claude Code runs as a persistent streaming session; Codex (`codex exec --json`) and Copilot CLI (`copilot --prompt`) are single-shot (one turn per session). The hosted browser UI has no chat runner; server-hosted vaults are reached by external agents over MCP. See ADR 0011.
