...
- **Embedded agent chat** *(implemented — ADR 0011)* — A desktop-only feature: the Tauri shell drives an agent (Claude Code, Codex, or Copilot) locally over the **Agent Client Protocol** and streams it to a chat panel over Tauri IPC. ACP is the **single transport** (ADR 0011 Phase E, fully implemented): the `notesmith-agent` crate exposes one `AcpSession` behind the `AgentSession` event stream and auto-wires the agent to the active vault's `/mcp/<vault>` (or read-only `/mcp-ro/<vault>`) endpoint via the ACP `session/new` `mcpServers` param, with a read-only default and a read-only/read-write toggle that maps onto ACP `session/request_permission`. Copilot speaks ACP natively (`copilot --acp`); Claude Code and Codex run over the same protocol via adapter binaries (`npx @zed-industries/claude-code-acp` and `codex-acp`), and a missing adapter yields a clear setup error rather than hanging. The agent kinds are `claude-code`, `codex`, and `copilot`. The hosted browser UI has no chat runner; server-hosted vaults are reached by external agents over MCP. See ADR 0011.
- **Agent vault access** *(MCP-first — ADR 0012)* — Because ACP proxies an agent's filesystem/terminal through the client, every session opens with a one-time context preamble steering the agent to the vault-aware MCP tools, and the agent has **no** direct shell/filesystem access by default. A global `agent.local_file_access` setting (off by default) advertises and implements scoped ACP `fs/*` and `terminal/*` handlers (`ClientHandler` in `notesmith-agent`), confined to the vault directory and refused in read-only sessions, for users who want the agent to read/write files and run commands directly. The desktop chat panel surfaces a per-session "Local file access" toggle (initialized from the global default) alongside the read-only toggle; CLI parity: `notesmith agent run --local-file-access`. See ADR 0012.
