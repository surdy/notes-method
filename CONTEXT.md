...
- **Theme System** — Theme state lives in `localStorage` under `notesmith:theme` as `{ theme, mode, visualMode, resolvedTone }`. A blocking inline script in `app.html` applies layered `data-theme`, `data-tone`, and `data-mode` attributes on `<html>` before paint so the generated ramp CSS can feed semantic tokens without flash, while `theme.svelte.ts` keeps those attributes in sync at runtime and temporarily mirrors legacy theme classes during the migration.
- **Settings Page** — A dedicated `/settings` route with left sidebar navigation and right content area for editing vault config in-app. Sections: General (name, homepage, capture folder/template), Daily Notes, Editor, Git, Hooks, Appearance (visual theme gallery grouped into dark/light sections, tone mode buttons, and a high-contrast toggle). Per-section Save/Revert with ETag-based conflict detection.
