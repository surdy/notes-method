...
- **MCP server** — Exposes the vault to MCP clients in two ways: the `notesmith mcp start` stdio server (builds its own in-memory indexes), and daemon-hosted streamable-HTTP endpoints mounted per vault at `/mcp/<vault>` (full) and `/mcp-ro/<vault>` (read-only) that reuse the daemon's live indexes. Both map MCP tool/resource requests onto the shared `notesmith-ops` operations layer (`NotesmithMcp` wraps an `Arc<dyn Ops>`). See ADR 0010.
