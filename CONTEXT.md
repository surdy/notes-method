...
- **Embedded agent chat** *(planned — ADR 0011)* — A desktop-only feature: the Tauri shell spawns an agent CLI (Copilot CLI, Claude Code, Codex) locally and streams it to a chat panel over Tauri IPC. A planned `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream and auto-wires the agent to the active vault's `/mcp/<vault>` (or read-only `/mcp-ro/<vault>`) endpoint. The hosted browser UI has no chat runner; server-hosted vaults are reached by external agents over MCP. See ADR 0011.
