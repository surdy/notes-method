...
- **Embedded agent chat** *(implemented — ADR 0011)* — A desktop-only feature: the Tauri shell spawns an agent CLI (Claude Code, Codex, or Copilot CLI) locally and streams it to a chat panel over Tauri IPC. The `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream and auto-wires the agent to the active vault's `/mcp/<vault>` (or read-only `/mcp-ro/<vault>`) endpoint, with a read-only default and a read-only/read-write toggle. Claude Code runs as a persistent streaming session; Codex (`codex exec --json`) and Copilot CLI (`copilot --prompt`) are single-shot (one turn per session). Copilot is additionally available over the **Agent Client Protocol** (`copilot --acp`, the `copilot-acp` kind / "Copilot (ACP)" picker option) — a multi-turn JSON-RPC transport that is the single-transport convergence point for Phase E; MCP wiring moves into the ACP `session/new` `mcpServers` param and the read-only/read-write scope maps onto ACP `session/request_permission`. Claude Code and Codex are also drivable over the same ACP transport (the `claude-acp` / `codex-acp` kinds) via adapter binaries (`npx @zed-industries/claude-code-acp` and `codex-acp`); a missing adapter yields a clear setup error rather than hanging. The hosted browser UI has no chat runner; server-hosted vaults are reached by external agents over MCP. See ADR 0011.
