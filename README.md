...
- **Three polished themes** — Dark, Light, and Split (dark chrome with a light editor), generated from `ui/app/src/styles/theme-catalog.json` with optional follow-system pairing, migration from the former catalog by tone, and a high-contrast overlay
...
- **Vault kits** — Install a blessed vault configuration (`.notesmith/` config, templates, dashboards, folder skeleton) instead of hand-authoring it: `notesmith kit apply work-notes`, or pick one under "Start from" when registering a vault. Non-destructive — existing files are reported and left alone, so it is safe on a populated vault and a no-op to re-run. The shipped [Work Notes kit](docs/example-work-notes-kit.md) is byte-identical to the `golden-vault/` fixture, so what you install is what the test suite exercises. See [ADR 0024](docs/adr/0024-vault-kits.md)
- **Input palette** — Sequential multi-step inputs for note creation and templates, including field pickers backed by the vault's own field values
...
- **Quill Rail file tree** — Folder-icon-free typographic hierarchy with disclosure carets, hairline branch connectors, optional per-note `_icon` emoji, and a compact search/new-note/new-folder/refresh toolbar
...
untouched code whenever clippy shipped a new lint. The weekly **Clippy Canary**
workflow runs the same gates against floating `stable`; it is non-blocking, and
red there means the next bump has work waiting, not that `main` is broken.
