...
#[tokio::test]
async fn inbox_list_shows_inbox_notes() {
    let temp_dir = TempDir::new().unwrap();
    let vault_root = temp_dir.path().join("work");
    create_vault(&vault_root, "work");
    // Pre-create an inbox note
    fs::create_dir_all(vault_root.join("Inbox")).unwrap();
    fs::write(
        vault_root.join("Inbox/2026-05-09 10-00-00 - Test Note.md"),
        "Test content",
    )
    .unwrap();
    ...
    assert!(
        !json.is_empty(),
        "expected at least one inbox note in listing"
    );
    assert!(json.iter().any(|note| {
        note["path"]
            .as_str()
            .unwrap_or_default()
            .contains("Test Note")
