...
> **GitHub Copilot gets stdio servers by a different route.** Copilot's ACP mode refuses any stdio MCP server handed to it by an ACP client — it logs `Rejecting non-http/sse MCP server "<id>" from client` and never starts the process (verified on Copilot CLI `1.0.83-1`; tracked upstream as [copilot-cli#3889](https://github.com/github/copilot-cli/issues/3889)). Copilot *does* accept stdio servers configured through its own config path, so for Copilot sessions Notesmith takes that path instead: it writes your enabled stdio entries to a private, per-session config file and starts Copilot with `--additional-mcp-config=@<that file>`. Those servers are then omitted from the ACP session setup, where they would only be refused. The result is that a stdio entry you add here works in **Copilot** sessions too — a stdio-only tool such as Microsoft's `workiq mcp` is usable from every agent (field-verified on Copilot CLI `1.0.83-3`). Nothing else changes: HTTP entries and Notesmith's built-in vault tools are still handed to every agent in its ACP session setup as before.
>
> Two details of the Copilot path are worth knowing. Each injected server is given a **55-second initialization timeout**, deliberately just under Copilot's hard 60-second budget for starting MCP servers ([copilot-cli#4421](https://github.com/github/copilot-cli/issues/4421)) — a server that is slow to start then fails on its own instead of taking the whole session down with it. And because the generated file can contain a server's environment variables (which may be credentials), it is written owner-read/write only (`0600`), its contents are never logged, and it is deleted as soon as the session ends.
>
> The full story — the limitation, how it bit the project, and the mechanism — is in [How Notesmith gets stdio MCP servers into Copilot](copilot-acp-stdio-workaround.md).
