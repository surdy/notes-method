...
- **New Window menu** — lists all vaults grouped by server (local + each remote), each row prefixed with a small native source icon (a computer for local vaults, a globe for remote ones) so you can tell them apart at a glance. Picking a vault opens a window connected to **that** server. The status-bar **badge** (bottom-left) shows the window's own connection — 💻 **This Mac** or ☁ the server name, with a live/offline dot for remotes. The sidebar vault label (top-left) also carries a source pill: "💻 Local" or "☁ <server>".
