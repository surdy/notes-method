...
- The new theme engine should map generated ramp primitives (`--neutral-*`, `--blue-*`, etc.) into a small shared semantic-token contract (`--bg-default`, `--text-default`, `--accent`, and peers). High-contrast should be an overlay on top of the default mapping, and `--text-inverse` is the one tone-specific exception because badge/accent backgrounds can flip polarity between dark and light themes.
