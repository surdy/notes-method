...
- **Embedded agent chat (implemented, see `docs/adr/0011-embedded-agent-chat.md`):** the desktop (Tauri) app runs an agent CLI (Claude Code, Codex, or Copilot CLI) as a chat panel, spawning it locally and streaming over Tauri IPC. The `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream; the spawned agent is auto-wired to the active vault's `/mcp/<vault>` (or `/mcp-ro/<vault>`, the default) endpoint, with a read-only/read-write toggle in the panel. Claude Code is a persistent streaming session; Codex and Copilot CLI are single-shot (one turn per session). Copilot is additionally available over the **Agent Client Protocol** (`copilot --acp`) — a multi-turn JSON-RPC transport and the single-transport convergence target of Phase E — where MCP wiring moves into the ACP `session/new` `mcpServers` param and the read-only/read-write scope maps onto ACP permission requests. Claude Code and Codex are also drivable over the same ACP transport (the `claude-acp` / `codex-acp` kinds) via adapter binaries (`npx @zed-industries/claude-code-acp` and `codex-acp`), with a graceful setup error when an adapter is not installed. Embedded chat is **desktop-only**; the hosted browser UI has no chat runner — server-hosted vaults are reached by external agents over MCP. Notesmith never manages model credentials.
