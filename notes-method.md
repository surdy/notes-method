...
- **Embedded agent chat (implemented, see `docs/adr/0011-embedded-agent-chat.md`):** the desktop (Tauri) app runs an agent (Claude Code, Codex, or Copilot) as a chat panel, driving it locally over the **Agent Client Protocol** and streaming over Tauri IPC. ACP is the **single transport** (ADR 0011 Phase E, fully implemented): the `notesmith-agent` crate exposes one `AcpSession` behind an `AgentSession` event stream; the agent is auto-wired to the active vault's `/mcp/<vault>` (or `/mcp-ro/<vault>`, the default) endpoint via the ACP `session/new` `mcpServers` param, with a read-only/read-write toggle in the panel that maps onto ACP permission requests. Copilot speaks ACP natively (`copilot --acp`); Claude Code and Codex run over the same protocol via adapter binaries (`npx @zed-industries/claude-code-acp` and `codex-acp`), with a graceful setup error when an adapter is not installed. The agent kinds are `claude-code`, `codex`, and `copilot`. Embedded chat is **desktop-only**; the hosted browser UI has no chat runner — server-hosted vaults are reached by external agents over MCP. Notesmith never manages model credentials. Vault access is **MCP-first** (see `docs/adr/0012-agent-local-io.md`): every session opens with a context preamble steering the agent to the vault-aware MCP tools, and the agent has no direct shell/filesystem access unless the opt-in `agent.local_file_access` setting (off by default) is enabled, which grants scoped ACP `fs/*` and `terminal/*` access confined to the vault directory and refused in read-only sessions.
