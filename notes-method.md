...
- When the desktop shell is pointed at a remote daemon with `NOTESMITH_DESKTOP_DAEMON_URL`, that daemon must serve the SvelteKit app at `/app/`; container deployments should use the `app` image flavor, not the binary-only `api` flavor, until the desktop shell embeds frontend assets locally.
