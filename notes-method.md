...
- **Embedded agent chat (planned, see `docs/adr/0011-embedded-agent-chat.md`):** the desktop (Tauri) app will be able to run an agent CLI (Copilot CLI, Claude Code, Codex) as a chat panel, spawning it locally and streaming over Tauri IPC. A `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream; the spawned agent is auto-wired to the active vault's `/mcp/<vault>` (or `/mcp-ro/<vault>`, the default) endpoint. Embedded chat is **desktop-only**; the hosted browser UI has no chat runner — server-hosted vaults are reached by external agents over MCP. Notesmith never manages model credentials.
