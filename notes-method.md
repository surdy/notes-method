...
- **Embedded agent chat (implemented, see `docs/adr/0011-embedded-agent-chat.md`):** the desktop (Tauri) app runs an agent CLI (Claude Code, Codex, or Copilot CLI) as a chat panel, spawning it locally and streaming over Tauri IPC. The `notesmith-agent` crate normalizes each CLI behind an `AgentSession` event stream; the spawned agent is auto-wired to the active vault's `/mcp/<vault>` (or `/mcp-ro/<vault>`, the default) endpoint, with a read-only/read-write toggle in the panel. Claude Code is a persistent streaming session; Codex and Copilot CLI are single-shot (one turn per session). Embedded chat is **desktop-only**; the hosted browser UI has no chat runner — server-hosted vaults are reached by external agents over MCP. Notesmith never manages model credentials.
