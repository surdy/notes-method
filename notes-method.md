...
- The app should default to the `notesmith-dark` catalog theme, with appearance state split into three layers: a catalog theme name (`data-theme`), a resolved tone (`data-tone` from dark/light/system), and a visual mode (`data-mode`, including a high-contrast overlay). Theme preference should persist in `localStorage` and apply without flash on load, while legacy CSS class mirroring remains in place until components finish migrating off `--ns-*` tokens. The settings page should expose the catalog as a grouped visual gallery with live preview swatches, and the command palette should offer a theme-picker sub-mode with preview-on-navigation before confirmation.
