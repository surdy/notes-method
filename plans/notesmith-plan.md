...
The desktop app is **local-only by default** and connects to remote daemons through an in-app **server list** (`servers.json`), managed in **Settings → Connection**. Connections are **per-window** (ADR 0017): the "New Window" menu lists vaults grouped by server, and each window owns its connection for its lifetime; the status-bar **badge** shows that window's own connection (local/remote, live/offline) without retargeting the app. When a window is connected to a remote server, the shell loads bundled SvelteKit assets from the `notesmith-app://localhost/app/` custom protocol and passes `apiBase=<daemon>` to the frontend. API calls and SSE streams target that daemon, so the desktop app works with both container flavors; only browser access requires the `app` flavor's daemon-served `/app/`. `servers.json`'s `active_id` is a **non-destructive default** (drives new-window / menu defaults), never retargeting open windows. See ADR 0017 (amending ADR 0014).
...
Theme assets are generated at build time from the deliberately small `ui/app/src/styles/theme-catalog.json` by the `theme-gen` workspace binary. Notesmith ships exactly three first-party themes: **Dark** (Graphite Precision), **Light** (Porcelain), and **Split** (Studio: dark chrome with a light editor). The generator writes `dark.css`, `light.css`, and `split.css` with 12-step neutral and ANSI hue ramps interpolated in OKLab/OKLCH space. Each theme also emits explicit semantic surface and border overrides for the approved sidebar, chrome, tab, input, and separator colors; these design-critical surfaces are not inferred from generic ramp positions. Split includes an explicit editor palette and editor semantic overrides under `[data-theme="split"] .editor-surface` so the writing surface can be tuned independently from the outer chrome. Settings → Appearance exposes the three choices through compact representative workspace previews and concise descriptions; obsolete catalog author/tag metadata is not part of the theme model. The command-palette theme picker exposes the same choices. Follow-system can pair Dark or Split with the fixed Light appearance. Persisted themes from the retired catalog migrate by former tone, with Manuscript mapping to Split.

### 19.2 Sidebar views

Sidebar views are user-defined in `.notesmith/sidebar.yaml`. By default (no YAML file), the sidebar shows only the Files tab — a folder-icon-free Quill Rail tree with disclosure carets, hairline vertical rails, short Branch Spine connectors on nested rows, and a narrow accent spine on the selected note. Generic document icons are omitted; an explicit per-note `_icon` remains visible. A compact toolbar combines note search with New Note, New Folder, and Refresh actions. New Note uses the shared title/folder input flow; New Folder creates or opens the folder's same-name folder note so the notes-derived tree can show the folder immediately. No tab bar is rendered.
