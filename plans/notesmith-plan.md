...
Theme assets are generated at build time from the deliberately small `ui/app/src/styles/theme-catalog.json` by the `theme-gen` workspace binary. Notesmith ships exactly three first-party themes: **Dark** (Graphite Precision), **Light** (Porcelain), and **Split** (Studio: dark chrome with a light editor). The generator writes `dark.css`, `light.css`, and `split.css` with 12-step neutral and ANSI hue ramps interpolated in OKLab/OKLCH space. Each theme also emits explicit semantic surface and border overrides for the approved sidebar, chrome, tab, input, and separator colors; these design-critical surfaces are not inferred from generic ramp positions. Split includes an explicit editor palette and editor semantic overrides under `[data-theme="split"] .editor-surface` so the writing surface can be tuned independently from the outer chrome. Settings → Appearance and the command-palette theme picker expose only these three choices. Follow-system can pair Dark or Split with Light. Persisted themes from the retired catalog migrate by former tone, with Manuscript mapping to Split.
