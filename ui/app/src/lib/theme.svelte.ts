...
const LEGACY_CLASSES = ['theme-dark', 'theme-light', 'theme-manuscript', 'theme-hc-dark'] as const;
...
function legacyClassFor(state: ThemeState): (typeof LEGACY_CLASSES)[number] {
	if (state.visualMode === 'high-contrast') return 'theme-hc-dark';
	if (state.theme === 'manuscript') return 'theme-manuscript';
	return state.resolvedTone === 'light' ? 'theme-light' : 'theme-dark';
}
...
function applyToDOM(state: ThemeState): void {
	...
	html.classList.remove(...LEGACY_CLASSES);
	html.classList.add(legacyClassFor(state));
